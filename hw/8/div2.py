...
class Div2:
    ...
    def __init__(self,lst, x, y, ctypes, key_fn = same):
        self.ctypes = ctypes
        #self."" = column_name_fn    #Part of the reason to use this is to check the column name in order to skip the column
        self.key_fn = key_fn    
    ...
    def divide(self,rank,low,high,b4):
        ...
        for i in range(low,high):
            right[self.x].addVal(self.b4[self.x].all_values[i])
            right[self.y].addVal(self.b4[self.y].all_values[i])
        
        for j in range(low, high):
            left[self.x].addVal(self.b4[self.x].all_values[j])
            left[self.y].addVal(self.b4[self.y].all_values[j])
            right[self.x].removeVal(self.b4[self.x].all_values[j])
            right[self.y].removeVal(self.b4[self.y].all_values[j])
        ...
        if cut:
            ...
            for each in range(len(low_b4)):
                for x in range(low,cut):
                    low_b4[each].addVal(self.b4[each].all_values[x])

            for each in range(len(high_b4)):
                for x in range(cut,high):
                    high_b4[each].addVal(self.b4[each].all_values[x])
